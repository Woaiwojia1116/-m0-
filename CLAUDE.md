...
| 子工程 | 芯片 | IDE | 用途 |
|--------|------|-----|------|
| `m0/` | TI MSPM0G3507 (Cortex-M0+) | Code Composer Studio | 智能小车巡线（位置式 PID 双环控制） |
...
**m0**：1ms 定时器中断读灰度 → 解算偏差 → 40ms 周期读编码器 → 双环 PID（速度环 + 巡线环）→ 左右电机 PWM 输出。编码器脉冲通过 GPIO 边沿中断累加。
...
`PID_Init(&V_PID, xunji, ...)` 使用的是名为 `xunji` 的参数数组 `{0,0,0}`，而 `PID_Init(&xunji_PID, _speed, ...)` 使用的是名为 `_speed` 的数组 `{1,1,0}`。**不要按名字推断用途**，看实际传值和 `Actual` 赋值位置。
