...
PID_para xunji[3]  = {1,1,0};   // 循迹环实际使用的PID参数
...
void oled_display(void)
{
    ...
    OLED_ShowNum(72,    0, V_PID.Actual, 3,OLED_8X16);
    // OLED_ShowString(0,  32, "lab_cnt", OLED_8X16);
    OLED_ShowNum(72,    16, V_PID.Target, 3, OLED_8X16);
     OLED_ShowNum(0,    0, V_PID.out, 3, OLED_8X16);
     OLED_ShowNum(0,    16, Encoder_count, 3, OLED_8X16);
     OLED_ShowNum(0,    32, Encoder2_count, 3, OLED_8X16);
...
void Control(void)
{
    ...
    else 
    {
        ...
        Moter_para[0] = V_PID.out;   // 左电机
        Moter_para[1] = V_PID.out;   // 右电机  
    }
