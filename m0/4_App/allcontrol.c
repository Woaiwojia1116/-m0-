...
PID_para _speed[3] = {0,0,0};   // 速度环实际使用的PID参数
...
{
    KeyEvent_t k[3] = {0};
    static uint8_t keyslow_cnt = 0;//减速计数
...
void oled_display(void)
{
    if (T.bits.oled_flag)
    {
        ...
        oled_slow++;
        if (oled_slow >= 100)   // 100ms 刷新一次（约 10Hz）
        {
            oled_slow = 0;
            OLED_ShowString(0,0,"carmode",OLED_8X16);
            OLED_ShowNum(72,0,CM.M1,1,OLED_8X16);
            OLED_ShowString(0,32,"lab_cnt",OLED_8X16);
            OLED_ShowNum(72,32,CM.lab_cnt,1,OLED_8X16);
            OLED_Update();
        }
    }
}
