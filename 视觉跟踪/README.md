# 视觉跟踪 — STM32F103 摄像头云台跟踪

电赛竞赛子工程。**STM32F103C8T6**（Cortex-M3）摄像头云台跟踪系统：摄像头识别目标位置 → **增量式 PID** → 控制两个 Emm V5.0 闭环步进电机制导云台对准目标。速度控制模式。
...
│   ├── PID.h/c           ← 增量式 PID
│   └── ...
├── LIB/                  ← STM32F10x StdPeriph 库 V3.5.0（只读，不改）
├── CMSIS/                ← Cortex-M3 内核、启动文件、system_stm32f10x（只读，不改）
└── PRJ/                  ← Keil 工程文件 + 构建输出（gitignore）
```
...
两个 `IRQHandler` 都写在 `BSP/usart.c` 里，不在 `APP/stm32f10x_it.c`（那里面只有空异常桩）。
...
```
上电 delay_ms(2000) 等待 Emm V5.0 模块自检就绪
   │
   └─ 主循环 7ms 周期轮询 Emm_Pro()
         │
         ├─ usart2_get_complete()==1
         │     ├─ down.pid_struct.Actual = (RX_DATA[0] - Center_position_x) * PULSE_PER_UNIT_X   // RX_DATA[0] → down
         │     ├─ over .pid_struct.Actual = (RX_DATA[1] - Center_position_y) * PULSE_PER_UNIT_Y   // RX_DATA[1] → over
         │     │
         ...
         │             motor_id=1 → down（水平/X）
         │             motor_id=2 → over（垂直/Y）
         │
         └─ 每条速度指令间插入 1ms 延时保证串口发送完成
```

### ⚠️ 电机命名与双层交叉陷阱（修改跟踪效果必先看）

结构体命名与运动方向**相反**：

| 结构体 | 实际运动轴向 | 电机 motor_id |
|---|---|---|
| `over` | **水平 X 轴** | 1 |
| `down` | **垂直 Y 轴** | 2 |

即 `over` 其实是 X 轴、`down` 其实是 Y 轴——**名字与轴向相反**。

此外，Actual 赋值也有**一层交叉**（`RX_DATA[0]`→down、`RX_DATA[1]`→over），`Emm_Pro()` 中电机速度又存在**第二层交叉**（motor_id=1 速度取自 `down.pid_struct.out`，motor_id=2 取自 `over.pid_struct.out`，direction 不交叉）。这两层交叉叠加，修改跟踪效果时极易搞错。

**以 `Emm_V5_Vel_Control` 实际调用行为准**，修改前先在纸上画出完整数据流。
...
- `dir`：0=CW，非 0=CCW
- `vel`：0~5000 RPM（`Emm_V5.h` 注释如此，实际受 PID ±1000 及 /20 缩放约束，约 ±50 RPM）
- `acc`：0~255，0 为直接启动
